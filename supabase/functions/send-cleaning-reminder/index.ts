...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: tasks, error: tasksError } = await supabase
      .from("cleaning_tasks")
      .select("id, name, last_done_at, interval_days, last_reminder_sent_at");
    if (tasksError) throw tasksError;

    const now = Date.now();
    // Nooit gedaan = altijd te doen (matcht de rode/"Te doen"-status in de
    // app); anders: te doen zodra last_done_at + interval_days verstreken is.
    const due = (tasks ?? []).filter((t) => {
      if (isSameLocalDay(t.last_reminder_sent_at)) return false;
      if (!t.last_done_at) return true;
      const dueAt = new Date(t.last_done_at).getTime() + t.interval_days * 24 * 60 * 60 * 1000;
      return dueAt <= now;
    });

    if (due.length === 0) {
      return new Response(JSON.stringify({ skipped: "no cleaning tasks due" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    // Voorkomt duplicaatmeldingen voor dezelfde taak binnen dezelfde dag —
    // zelfde aanpak als plant_instances.last_water_reminder_sent_at.
    ...
    const payload = JSON.stringify({
      title: `Schoonmaak: ${due.length} ${due.length === 1 ? "taak" : "taken"} te doen`,
      body: namesList(due.map((t) => t.name)),
      url: "/#/schoonmaak",
    });
    ...
    console.error(err);
    return new Response(
      JSON.stringify({ error: String(err), detail: err instanceof Error ? err.message : JSON.stringify(err) }),
