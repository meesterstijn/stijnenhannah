...
export default function GameNightGuest() {
  ...
  const update = useMutation({
    mutationFn: async ({
      ...
      if (photo && guestToken && sessionId)
        await saveGuestFace(guestToken, sessionId, photo);
    },
    onSuccess: async () => {
  ...
  return (
    <GuestFaceAccessContext.Provider value={{ guestToken, sessionId }}>
          ...
          {!data?.valid ? (
            !state.isError && (
              <div className="gnv2-guest-panel">
                ...
                <p>
                  Scan de uitnodiging van de host en kies je opgeslagen speler.
                  Je hoeft je naam en avatar niet opnieuw te maken.
                </p>
              </div>
            )
              ...
              <section className="gnv2-guest-panel gnv2-guest-welcome">
                ...
                >
                  <GuestAvatar
                    ...
                    body={data.me.body}
                    photoPath={data.me.face_asset_path}
                    photoRevision={data.me.face_revision}
                  />
                </div>
                <div>
