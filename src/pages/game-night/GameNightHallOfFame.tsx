...
import { cardTilt } from "@/features/game-night/lib/gameCoverPlaceholder";
...
function GameAwardPlaque({ award }: { award: GameAwardResult }) {
  ...
  const tilt = cardTilt(award.config.slug);
  ...
  return (
    ...
    >
      ...
      {award.holders.length > 0 ? (
        <>
          <p
            className="gn-display mt-1.5 text-xl font-semibold"
            style={{ color: "var(--gn-brass)" }}
          >
        ...
        <>
          ...
          <p className="gn-faint mt-1 text-xs">{award.requirementText}</p>
        </>
      )}
      {award.game && (
        <Link
...
function GeneralAwardCard({ award }: { award: GeneralAwardResult }) {
  return (
    <div className="gn-panel-elevated px-5 py-4 text-center">
      <p className="gn-display text-base font-semibold tracking-wide">
        {award.config.title}
      </p>
      <p className="gn-faint mt-0.5 text-xs">{award.config.description}</p>
      {award.holders.length > 0 ? (
        <p
        ...
        <p className="gn-faint mt-2 text-xs italic">Nog niet uitgereikt</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8">
      ...
      <div className="text-center">
        ...
        <p className="gn-muted mt-1.5 text-sm">De eeuwige roem</p>
      </div>
      ...
      {isLoading || !data || !records ? (
        ...
        <>
          {gameAwards.length > 0 && (
            <div>
              <p className="gn-eyebrow mb-3 text-center">Titelhouders</p>
          ...
          {generalAwards.length > 0 && (
            <div>
              <p className="gn-eyebrow mb-3 text-center">Speelse titels</p>
          ...
          <div className="gn-panel-elevated px-5 py-4">
            <p className="gn-eyebrow mb-1">Records</p>
            <div
              className="divide-y"
            ...
            {[
              ...
              <p className="gn-faint text-sm">
                Nog geen records — speel wat Game Nights om deze lijst te
                vullen.
              </p>
            )}
          </div>
