...
import type { CocktailFull, CocktailVariantFull } from "@/features/cocktail-bar/types";
...
export function CocktailDetailDialog({
  ...
  const [variantType, setVariantType] = useState<"alcoholic" | "alcohol_free">("alcoholic");

  const alcoholicVariant = cocktail?.variants.find((v) => v.variant_type === "alcoholic") ?? null;
  const alcoholFreeVariant = cocktail?.variants.find((v) => v.variant_type === "alcohol_free") ?? null;
  const activeVariant: CocktailVariantFull | null =
    (variantType === "alcoholic" ? alcoholicVariant : alcoholFreeVariant) ?? alcoholicVariant ?? alcoholFreeVariant;

  const photoPath = activeVariant?.photo_storage_path ?? cocktail?.photo_storage_path ?? null;
    ...
    <Dialog open={!!cocktail} onOpenChange={(open) => !open && onClose()}>
      {cocktail && activeVariant && (
        <DialogContent className="cocktail-theme cb-dialog w-full max-w-3xl max-h-[90vh] sm:rounded-3xl">
          <DialogHeader>
            <DialogTitle asChild>
              <h2 className="cb-heading font-serif text-3xl sm:text-4xl">{cocktail.name}</h2>
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-6">
            <div className="relative flex h-56 w-full items-center justify-center rounded-2xl sm:h-72">
              ...
              {photoUrl ? (
                <img src={photoUrl} alt={cocktail.name} className="relative h-full w-full object-contain" />
              ) : (
                <Martini className="relative h-20 w-20 text-[var(--cb-gold)] opacity-50" strokeWidth={1.2} />
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              <span className="cb-badge">
                {activeVariant.variant_type === "alcohol_free" ? "Alcoholvrij" : activeVariant.spirit?.name ?? "Onbekend"}
              </span>
              {badges.map((b) => (
            ...
            <div>
              ...
              <CocktailFlavourProfileChart profile={activeVariant.flavour_profile} />
            </div>

            <div className="space-y-3">
              <h3 className="cb-heading mb-2 text-lg">Ingrediënten</h3>
              <CocktailIngredientList ingredients={activeVariant.ingredients} />
              <CocktailAddToGroceryListButton ingredients={activeVariant.ingredients} />
            </div>

            <div>
              <h3 className="cb-heading mb-2 text-lg">Bereiding</h3>
              <p className="text-sm leading-relaxed">{activeVariant.preparation_steps}</p>
            </div>
            ...
            {cocktail.backstory && (
              <div>
                ...
                <p className="cb-muted text-sm leading-relaxed">{cocktail.backstory}</p>
              </div>
            )}
          </div>
        </DialogContent>
