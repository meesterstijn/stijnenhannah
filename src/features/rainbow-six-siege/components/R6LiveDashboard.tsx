...
import { R6LiveScoreboard } from "@/features/rainbow-six-siege/components/R6LiveScoreboard";
...
// Een "Game" is één volledige Rainbow Six-map — bewust niet "ronde" genoemd
// (dat woord gebruikt Rainbow Six zelf al voor de rondes ÍN een map, wat
// alleen maar verwarring geeft). Een Game is puur een container waarin
// gebeurtenissen verzameld worden; "Game afronden" (zie R6EndGameSheet) is
...
export function R6LiveDashboard({
  ...
  // Ranglijst bovenaan sorteert live mee (dat is precies het punt), maar de
  // tik-kaarten hieronder staan bewust in vaste rooster-volgorde: een kaart
  // die van plek wisselt net nadat je 'm hebt aangetikt zou de volgende tik
  // juist moeilijker maken, terwijl "binnen één seconde registreren" het doel is.
  const entryByPlayerId = new Map(scoreboard.map((entry) => [entry.player.id, entry]));

  return (
    <div className="space-y-4">
      <R6LiveScoreboard entries={scoreboard} />

      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-zinc-400">{currentMatch ? `Game ${currentMatch.match_number} bezig` : "Game wordt gestart…"}</p>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        {roster.map((player) => {
          ...
          return (
            <div key={player.id} className="space-y-3 rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4">
      ...
      <section className="space-y-2">
        <p className="text-sm font-medium text-zinc-300">Laatste acties (deze game eerst)</p>
        <R6RecentEventsFeed
